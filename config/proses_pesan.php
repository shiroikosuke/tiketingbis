<?php
	...
	$query 			= "SELECT * FROM kursi_pesanan WHERE id_bus='$id_bus' AND tgl_berangkat ='$tgl_berangkat' ";
	$cek    	    = mysqli_query($conn, $query)or die(mysqli_error($conn));
	$jumlah			= mysqli_num_rows($cek);
	$totalpesan = $jumlah+$penumpang;
	...
	if ($totalpesan >= 55) {
		...
		echo "<META HTTP-EQUIV='REFRESH' CONTENT ='0; URL=../index.php?halaman=kursi&&id_pesan=$idp&&tgl_berangkat=$tgl_berangkat&&penumpang=$penumpang&&idbus=$id_bus'>";
	}
?>